...
public class RequestParams implements Serializable {
    ...
    protected final  ConcurrentHashMap<String, ArrayList<FileWrapper>> files=new ConcurrentHashMap<String, ArrayList<FileWrapper>>(); 
    public void addFile(String key,File file){
    		if(files.containsKey(key)){
    			files.get(key).add(new FileWrapper(file, null, null));
    		}else{
    			ArrayList<FileWrapper> list=new ArrayList<FileWrapper>();
    			list.add(new FileWrapper(file, null, null));
    			files.put(key, list);
    		}
    }
    ...
    public HttpEntity getEntity(ResponseHandlerInterface progressHandler) throws IOException {
        ...
        } else if (streamParams.isEmpty() && fileParams.isEmpty()&&files.isEmpty()) {
    ...
    private HttpEntity createMultipartEntity(ResponseHandlerInterface progressHandler) throws IOException {
        ...
        for(Entry<String, ArrayList<FileWrapper>> entry : files.entrySet()){
        		String key=entry.getKey();
        		ArrayList<FileWrapper> list= entry.getValue();
